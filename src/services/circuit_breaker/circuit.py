...
class CircuitBreaker:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        logger.info(f'CIRCUITBREAKER {self.name} находится в состоянии {self._state}')
        if exc_type:
            logger.error('CIRCUITBREAKER: Произошла ошибка во время обращения к функции!', exc_info=True)
        ...
        def wrapper(*args, **kwargs):
            if self.is_opened:
                if self._is_func_exec_after_open_state_finished():
                    ...
                    logger.warning(
                        f'CIRCUITBREAKER {self.name} будет в открытом состоянии еще {self.time_left_in_open_state}',
                    )
